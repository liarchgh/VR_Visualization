...
public class WMG_Random_Graph : WMG_Graph_Manager {
	...
	public List<GameObject> GenerateGraphFromNode(WMG_Node fromNode) {
		...
		while (NodesParent.Count - numNodesStarting < numNodes) {
			...
			if (debugRandomGraph) GlobalVariableBackground.Instance.conManager.WriteLog("Processesing Node: " + procNode.id + " with " + numNeighbors + " neighbors.");
			...
			for (int i = 0; i < numNeighbors; i++) {
				...
				while (curNeighborAttempt < maxNeighborAttempts) {
					...
					if (debugRandomGraph) GlobalVariableBackground.Instance.conManager.WriteLog("Neighbor: " + i + " Attempt: " + curNeighborAttempt + " angle: " + Mathf.Round(neighborAngle));
					...
					if (failedAttempt) {
						...
						if (debugRandomGraph) GlobalVariableBackground.Instance.conManager.WriteLog("Failed: Angle within minAngle of existing neighbor");
						curNeighborAttempt++;
						continue;
					}
					...
					if (noLinkIntersection) {
						...
						foreach (GameObject child in LinksParent) {
							...
							if (WMG_Util.LineSegmentsIntersect(p1x, p1y, p2x, p2y, p3x, p3y, p4x, p4y)) { // Links intersect
								if (debugRandomGraph) GlobalVariableBackground.Instance.conManager.WriteLog("Failed: Link intersected with existing link: " + childLink.id);
								failedAttempt = true;
								break;
							}
						}
					...
					if (noNodeIntersection) {
						...
						foreach (GameObject child in NodesParent) {
							...
							if (Mathf.Pow((p1x - child.transform.localPosition.x),2) + Mathf.Pow((p1y - child.transform.localPosition.y),2) <= Mathf.Pow(2*(procNode.radius + noNodeIntersectionRadiusPadding),2)) {
								if (debugRandomGraph) GlobalVariableBackground.Instance.conManager.WriteLog("Failed: Node intersected with existing node: " + aNode.id);
								failedAttempt = true;
								break;
							}
						}
					...
					if (noLinkNodeIntersection) {
						...
						foreach (GameObject child in NodesParent) {
							...
							if (WMG_Util.LineIntersectsCircle(p1x, p1y, p2x, p2y, child.transform.localPosition.x, child.transform.localPosition.y, aNode.radius + noLinkNodeIntersectionRadiusPadding)) {
								if (debugRandomGraph) GlobalVariableBackground.Instance.conManager.WriteLog("Failed: Link intersected with existing node: " + aNode.id);
								failedAttempt = true;
								break;
							}
						}
					...
					if (noLinkNodeIntersection) {
						...
						foreach (GameObject child in LinksParent) {
							...
							if (WMG_Util.LineIntersectsCircle(p1x, p1y, p2x, p2y, cx, cy, procNode.radius + noLinkNodeIntersectionRadiusPadding)) {
								if (debugRandomGraph) GlobalVariableBackground.Instance.conManager.WriteLog("Failed: Node intersected with existing link: " + childLink.id);
								failedAttempt = true;
								break;
							}
						}
			...
			if (NodesParent.Count - numNodesStarting == numNodesProcessed) { // Case where all nodes have been processed, but number nodes specfied were not created
				GlobalVariableBackground.Instance.conManager.WriteLog("WMG - Warning: Only generated " + (NodesParent.Count - numNodesStarting - 1) + " nodes with the given parameters.");
				break;
			}
		}
